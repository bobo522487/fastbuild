...
export class AnalyticsService {
    ...
    try {
      ...
      return results.map((row: any) => ({
        ...
        stats: {
          ...
          avgBuildTime: parseFloat(row.avg_build_time) || 0,
   ...
   * 获取部署统计分析
   */
  async getDeploymentStatistics(days: number = 30) {
    try {
      ...
      return {
        summary: stats ? {
          ...
          avgBuildTime: parseFloat(stats.avg_build_time) || 0,
          avgDeployTime: parseFloat(stats.avg_deploy_time) || 0,
          successRate: parseFloat(stats.success_rate) || 0,
          mostCommonFailureReason: stats.most_common_failure_reason,
          deploymentFrequency: parseFloat(stats.deployment_frequency) || 0,
    ...
    try {
      ...
      return results.map((row: any) => ({
        ...
        usage: {
          ...
          storageUsedMB: parseFloat(row.storage_used_mb) || 0,
          mostActiveTable: row.most_active_table,
          avgColumnsPerTable: parseFloat(row.avg_columns_per_table) || 0,
          dataGrowthRate: parseFloat(row.data_growth_rate) || 0
        }
      }));
    } catch (error) {
   ...
   * 获取增长趋势分析
   */
  async getGrowthTrends(days: number = 90) {
    try {
      ...
      return {
        users: userGrowth.map((row: any) => ({
          ...
          avgTimeBetweenUsers: parseFloat(row.avg_value) || 0
        })),
        projects: projectGrowth.map((row: any) => ({
          period: row.period,
          newProjects: parseInt(row.count),
          avgTimeBetweenProjects: parseFloat(row.avg_value) || 0
        })),
        applications: appGrowth.map((row: any) => ({
          period: row.period,
          newApplications: parseInt(row.count),
          avgTimeBetweenApplications: parseFloat(row.avg_value) || 0
        }))
      };
    } catch (error) {
  ...
  async getPerformanceBenchmarks() {
    try {
      ...
      return {
        ...
        recommendations: [
          '使用 PG_UNICODE_FAST 排序规则优化多语言查询',
          '为频繁查询的字段添加索引',
          '考虑分区表处理大数据量',
          '使用复合类型聚合减少数据传输'
        ]
