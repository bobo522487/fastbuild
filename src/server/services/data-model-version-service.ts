...
export class DataModelVersionService {
	...
	async createDataModelVersion(data: {
		...
		try {
			...
			const integrityHash = FastBuildIdGenerator.generateShortId('hash') + '_' +
				Buffer.from(content).toString('base64').slice(0, 16);

			// 使用标准 Prisma 方式创建版本
			const version = await this.prisma.dataModelDeployment.create({
				data: {
					id: FastBuildIdGenerator.generateDeploymentId(),
			...
			await this.prisma.auditLog.create({
				data: {
					...
					action: "CREATE_DATA_MODEL_DEPLOYMENT",
					resourceType: "DATA_MODEL_DEPLOYMENT",
	...
	async updateDataModelVersionStatus(
		...
		try {
			// 先获取当前版本信息
			const currentVersion = await this.prisma.dataModelDeployment.findUnique({
				where: { id },
			});
			...
			// 更新版本状态
			const version = await this.prisma.dataModelDeployment.update({
				where: { id },
				data: {
					status,
			...
			await this.prisma.auditLog.create({
				data: {
					...
					userId,
					action: "UPDATE_DATA_MODEL_DEPLOYMENT_STATUS",
					resourceType: "DATA_MODEL_DEPLOYMENT",
					resourceId: id,
					metadata: {
		...
		try {
			...
			if (version.metadata?.integrityHash) {
				...
				const content = JSON.stringify({
					...
					timestamp: version.deployedAt?.toISOString(),
				});
				const currentHash = FastBuildIdGenerator.generateShortId('hash') + '_' +
					Buffer.from(content).toString('base64').slice(0, 16);
			...
			// 2. 验证表快照结构
			if (
				!version.tableSnapshots ||
				Object.keys(version.tableSnapshots).length === 0
			) {
				issues.push("缺少表快照数据");
	...
	async compareVersions(
		...
		try {
			...
			const modified = common.filter((table) => {
				return (
					JSON.stringify(snapshots1[table]) !==
					JSON.stringify(snapshots2[table])
				);
			});
			...
			if (removed.length > 0) {
				compatibilityIssues.push(
					`删除了 ${removed.length} 个表，可能导致数据丢失`,
				);
			}
			...
			for (const tableName of modified) {
				...
				if (table1?.columns && table2?.columns) {
					...
					const removedFields = [...fields1].filter(
						(field) => !fields2.has(field),
					);
					if (removedFields.length > 0) {
						compatibilityIssues.push(
							`表 ${tableName} 删除了 ${removedFields.length} 个字段`,
						);
					}
				}
			}
			...
			const upgradeRequired =
				compatibilityIssues.length > 0 || removed.length > 0;
			...
			const versionsWithDifferences = await Promise.all(
				versions.map(async (version, index) => {
					...
					return {
						...
						buildTime: version.buildTime || undefined,
		...
		try {
			...
			await this.prisma.auditLog.create({
				data: {
					...
					projectId: targetVersion.projectId,
					userId,
					action: "ROLLBACK_DATA_MODEL_DEPLOYMENT",
					resourceType: "DATA_MODEL_DEPLOYMENT",
	...
	async getDataModelUsageStats(projectId: string): Promise<{
		...
		try {
			...
			const totalDeploysLast30Days = recentDeploys.length;

			if (recentDeploys.length > 1) {
				const lastDeploy = recentDeploys[recentDeploys.length - 1];
				const firstDeploy = recentDeploys[0];

				if (lastDeploy?.deployedAt && firstDeploy?.deployedAt) {
					const timeDiff =
						lastDeploy.deployedAt.getTime() -
						firstDeploy.deployedAt.getTime();
					avgDaysBetweenDeploys =
						timeDiff / (recentDeploys.length - 1) / (24 * 60 * 60 * 1000);
				}
			}

			const envCounts = environments.reduce(
				(acc, env) => {
