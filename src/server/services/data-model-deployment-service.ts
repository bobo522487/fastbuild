...
export type DataModelDeploymentRequest = z.infer<
	typeof DataModelDeploymentRequestSchema
>;
...
export class DataModelDeploymentService {
	async deployDataModel(
		...
		if (existingDeployment) {
			throw new Error(
				`Version ${validatedRequest.version} already exists for ${validatedRequest.environment} environment`,
			);
		}
	...
	async getDeploymentStatus(deploymentId: string, userId: string) {
		...
		// 检查权限
		const userMembership = deployment.project.members.find(
			(member) => member.userId === userId,
		);
		if (!userMembership) {
		...
		if (deployment.status === "BUILDING") {
			...
			response.estimatedCompletion = new Date(
				deployment.deployedAt.getTime() + averageBuildTime,
			).toISOString();
		}
