...
export class PerformanceMonitorService {
  ...
  async enableEnhancedStatistics(): Promise<void> {
    try {
      ...
      await this.prisma.$executeRaw`ALTER SYSTEM SET track_cost_delay_timing = on`;
  ...
  async getPerformanceReport(): Promise<{
    ...
    const [ioStats, walStats, checkpointStats, vacuumStats, tableStats, indexStats, activeConnections] =
      await Promise.all([
  ...
  async checkPostgreSQLVersion(): Promise<{
    ...
    try {
      ...
      // 简单的版本检查（实际应该更精确）
      const isPG18OrHigher = version.includes('PostgreSQL 18') ||
