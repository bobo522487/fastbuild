...
export class PartitionService {
  ...
  async checkPartitionSupport(): Promise<{
    ...
    try {
      ...
      const partitionedTables = [...new Set(partitions.map((p: any) => p.tablename.split('_')[0]))];
   ...
   * 使用 ONLY 选项进行分区表维护
   */
  async vacuumPartitions(onlyCurrentMonth: boolean = true): Promise<{
    success: boolean;
    results: Array<{
      partitionName: string;
      operation: string;
   ...
   * 获取优化的查询建议
   */
  async getQueryOptimization(startDate?: Date, endDate?: Date): Promise<{
    ...
    try {
      ...
      return {
        recommendedQuery: 'SELECT * FROM "AuditLog" ORDER BY "createdAt" DESC LIMIT 100',
  ...
  async queryAuditLog(filters: {
    ...
    try {
      ...
      try {
        ...
        const results = await this.prisma.$queryRawUnsafe(query, ...queryParams) as any[];
  ...
  async getPartitionPerformanceStats(): Promise<{
    ...
    try {
      ...
      if (parseInt(partitionCount.count) > 15) {
        recommendations.push('分区数量较多，考虑清理旧分区以提升性能');
      }

      if (parseInt(partitionCount.count) < 6) {
        recommendations.push('分区数量较少，考虑创建更多分区以优化查询');
      }

      return {
        partitionEfficiency: Math.min(100, parseInt(partitionCount.count) * 6),
