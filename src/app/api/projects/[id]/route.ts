		...
		return NextResponse.json(
			{
				...
				data: {
					...
					dataVersionCount: updatedProject._count.DataVersions,
					currentUserRole:
						updatedProject.members.find((member) => member.userId === user.id)
							?.role || null,
				},
				meta: {
					timestamp: new Date().toISOString(),
...
export async function DELETE(
	...
	try {
		...
		const hasDependencies =
			existingProject.applications.length > 0 ||
			existingProject.DataVersions.length > 0;
