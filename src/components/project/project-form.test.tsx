...
describe('ProjectForm 组件 - 单元测试', () => {
  ...
  describe('表单渲染', () => {
    ...
    it('AC1: 应该显示正确的占位符文本', () => {
      ...
      expect(screen.getByPlaceholderText('输入项目描述（可选）')).toBeInTheDocument();
    });
  ...
  describe('表单验证', () => {
    ...
    it('AC1: 应该验证特殊字符输入', async () => {
      ...
      // 测试包含特殊字符的项目名称
      fireEvent.change(nameInput, { target: { value: '项目<script>alert("xss")</script>' } });
      fireEvent.click(submitButton);
  ...
  describe('表单提交', () => {
    it('AC1: 应该成功创建项目', async () => {
      ...
      fireEvent.change(screen.getByLabelText('项目描述'), { target: { value: '这是一个测试项目' } });
    ...
    it('AC1: 应该处理创建失败', async () => {
      ...
      // Fill form
      fireEvent.change(screen.getByLabelText('项目名称'), { target: { value: '重复项目名' } });
    ...
    it('AC1: 应该处理网络错误', async () => {
      ...
      await waitFor(() => {
        expect(mockToast.error).toHaveBeenCalledWith('创建项目失败，请稍后重试');
      });
    });
    ...
    it('AC1: 应该处理服务器错误响应', async () => {
      ...
      // Fill form
      fireEvent.change(screen.getByLabelText('项目名称'), { target: { value: '无效项目名' } });
      fireEvent.change(screen.getByLabelText('项目描述'), { target: { value: '敏感描述内容' } });
  ...
  describe('用户体验', () => {
    it('AC1: 应该提供实时验证反馈', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('项目名称不能超过100个字符')).not.toBeInTheDocument();
      });
    });
    ...
    it('AC1: 应该使用测试数据工厂进行批量测试', async () => {
      ...
      const testProjects = TestDataFactory.createTestProjects(3, 'test-owner-id');
      ...
      for (const project of testProjects) {
        ...
        fireEvent.change(screen.getByLabelText('项目描述'), { target: { value: project.description } });
        fireEvent.change(screen.getByLabelText('可见性'), { target: { value: project.visibility } });
