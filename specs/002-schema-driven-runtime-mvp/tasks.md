...
### 🔄 阶段 1：修复关键错误 [高优先级]
- [ ] **任务 1.1**: 修复 tRPC 中间件类型错误
  - [ ] 修复 `packages/api/src/middleware/index.ts` 中的类型兼容性问题
  - [ ] 更新 tRPC 类型定义以支持 subscription 类型
  - [ ] 确保所有中间件函数类型正确

- [ ] **任务 1.2**: 修复测试配置问题
  - [ ] 修复 Playwright 测试配置冲突
  - [ ] 解决 Vitest CommonJS 模块导入问题
  - [ ] 修复 Prisma mock 函数类型错误
  - [ ] 更新测试设置文件格式

### 🔄 阶段 2：完善测试覆盖 [高优先级 - TDD宪法要求]
- [ ] **任务 2.1**: 表单组件单元测试
  - [ ] 为 `DynamicFormRenderer` 编写组件测试
  - [ ] 为所有字段类型编写渲染测试
  - [ ] 为表单验证逻辑编写测试
  - [ ] 为条件字段显示逻辑编写测试
...
- [ ] **任务 2.2**: 表单提交集成测试
  - [ ] 测试 console.log 输出功能
  - [ ] 测试 submissionRouter.create 调用
  - [ ] 测试提交成功/失败状态处理
  - [ ] 测试表单数据持久化

- [ ] **任务 2.3**: Schema 编译器性能测试
  - [ ] 测试 Schema 编译性能 (< 10ms 目标)
  - [ ] 测试缓存机制有效性
  - [ ] 测试复杂表单编译性能
  - [ ] 编写性能基准测试
...
#### 需要完善的原则
- [ ] **Test-Driven Development**: 当前缺少测试用例 (任务 2.1, 2.2, 2.3)
- [ ] **Performance Monitoring**: 需要添加性能监控 (任务 3.2)
