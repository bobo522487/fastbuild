...
export class E2ETestHelpers {
  ...
  static async createProject(
    ...
    // 等待项目创建完成
    await page.waitForSelector(`[data-testid="project-card-${projectName.toLowerCase().replace(/\\s+/g, '-')}"]`, {
      timeout: 10000,
    });
   ...
   * 模拟网络延迟
   */
  static async simulateNetworkDelay(page: Page, delayMs: number): Promise<void> {
   ...
   * 离线模式测试
   */
  static async setOfflineMode(page: Page, offline: boolean = true): Promise<void> {
   ...
   * 验证项目卡片存在
   */
  static async verifyProjectCard(page: Page, projectName: string): Promise<boolean> {
  ...
  static async getFormValidationErrors(page: Page): Promise<string[]> {
    const errorElements = await page.locator('[data-testid^="validation-error"]');
    const errors: string[] = [];
