...
// import { GET, POST } from "~/app/api/projects/route"; // API file deleted - test disabled

// Mock functions for disabled tests
const GET = vi.fn();
const POST = vi.fn();

describe.skip("项目 API - 集成测试 (API 文件已删除)", () => {
		...
		it("AC1: 应该验证项目名称长度", async () => {
			...
			const request = createMockRequest(
				...
				invalidProjectData,
				{
					"x-user-id": mockUser.id,
					"x-user-email": mockUser.email,
					"x-user-name": mockUser.name || "Test User",
				},
			);
	...
	describe("GET /api/projects", () => {
		...
		it("AC1: 应该支持可见性过滤", async () => {
			...
			const request = createMockRequest(
				"GET",
				"/api/projects?visibility=PUBLIC",
			);
			const response = await GET(request);
			const result = validateApiResponse(response);
			const data = await response.json();
