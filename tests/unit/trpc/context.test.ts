  ...
  describe('createContext', () => {
    ...
    describe('用户查找和处理', () => {
      it('应该处理用户不存在的情况', async () => {
        mockReq.headers.authorization = 'Bearer valid-token';
        vi.mocked(jwt.verify).mockReturnValue({ userId: 'nonexistent-user' } as any);
      ...
      it('应该处理非活跃用户', async () => {
        mockReq.headers.authorization = 'Bearer valid-token';
        vi.mocked(jwt.verify).mockReturnValue({ userId: 'inactive-user' } as any);
      ...
      it('应该处理缺少 userId 的令牌', async () => {
        mockReq.headers.authorization = 'Bearer valid-token';
        vi.mocked(jwt.verify).mockReturnValue({ email: 'test@example.com' } as any); // 缺少 userId
      ...
      it('应该正确映射用户角色', async () => {
        mockReq.headers.authorization = 'Bearer valid-token';
        vi.mocked(jwt.verify).mockReturnValue({ userId: 'admin-user' } as any);
