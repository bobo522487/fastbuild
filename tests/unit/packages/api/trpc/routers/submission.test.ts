...
describe('tRPC Submission Router 单元测试', () => {
  ...
  describe('create', () => {
    it('应该成功创建表单提交', async () => {
      ...
      (mockPrisma.form.findUnique as any).mockResolvedValue(mockForm);
    ...
    it('应该拒绝提交到不存在的表单', async () => {
      (mockPrisma.form.findUnique as any).mockResolvedValue(null);
  ...
  describe('getById', () => {
    it('提交者应该能查看自己的提交详情', async () => {
      ...
      (mockPrisma.submission.findUnique as any).mockResolvedValue(mockSubmission);
    ...
    it('应该拒绝查看不存在的提交', async () => {
      (mockPrisma.submission.findUnique as any).mockResolvedValue(null);
  ...
  describe('getByFormId', () => {
    it('表单创建者应该能查看表单的所有提交', async () => {
      ...
      (mockPrisma.form.findUnique as any).mockResolvedValue(mockForm);
      (mockPrisma.submission.findMany as any).mockResolvedValue(mockSubmissions);
  ...
  describe('getStats', () => {
    it('应该获取提交统计信息', async () => {
      (mockPrisma.submission.count as any).mockResolvedValue(100); // totalSubmissions
      (mockPrisma.submission.count as any).mockResolvedValue(20); // recentSubmissions
    ...
    it('应该支持按表单过滤统计', async () => {
      ...
      (mockPrisma.form.findUnique as any).mockResolvedValue(mockForm);
      (mockPrisma.submission.count as any).mockResolvedValue(50); // totalSubmissions
      (mockPrisma.submission.count as any).mockResolvedValue(10); // recentSubmissions
      (mockPrisma.submission.findMany as any).mockResolvedValue([
        { createdAt: new Date() },
      ]);
