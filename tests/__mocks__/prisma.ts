...
export const createMockPrismaClient = () => {
	...
	const createMockOperations = (entityName: keyof typeof mockData) => ({
		...
		findUnique: vi.fn().mockImplementation(async ({ where }: any) => {
			const entity = mockData[entityName].find((item: any) => {
		...
		findMany: vi
			.fn()
			.mockImplementation(async ({ where, take, skip, orderBy }: any = {}) => {
						...
						return Object.entries(where).every(([key, value]: [string, any]) => {
							if (typeof value === "object" && value !== null) {
								...
								return Object.entries(value).every(([op, opValue]: [string, any]) => {
									if (op === "contains") {
										return item[key]?.includes(opValue) ?? false;
									}
		...
		update: vi.fn().mockImplementation(async ({ where, data }: any) => {
			const index = mockData[entityName].findIndex((item: any) =>
		...
		aggregate: vi
			.fn()
			.mockImplementation(async ({ _count, _avg, _sum, _min, _max }: any = {}) => {
				...
				if (_sum) {
					...
					for (const field of Object.keys(_sum)) {
						const values = results
							.map((r: any) => Number(r[field]))
							.filter((v: any) => !isNaN(v));
						aggregateResult._sum[field] = values.reduce((a: any, b: any) => a + b, 0);
					}
				}
	...
	return {
		...
		// Helper functions
		resetMockData: () => {
			Object.keys(mockData).forEach((key: keyof typeof mockData) => {
				mockData[key] = [];
			});
		},
...
export const { mockPrismaClient, mockData, resetMockData } =
