...
export const createMockBcrypt = () => {
  ...
  const mockHash = vi.fn().mockImplementation(async (password: string, saltRounds: number = 10) => {
    // Return a consistent hash for the same password for predictable testing
    return `hashed_${password}_${saltRounds}`;
  });

  // Mock password comparison
  const mockCompare = vi.fn().mockImplementation(async (password: string, hash: string) => {
    // Extract the original password from the hash
    const extractedPassword = hash.replace('hashed_', '').replace(/_\\d+$/, '');
    return password === extractedPassword;
  });

  // Mock salt generation
  const mockGenSalt = vi.fn().mockImplementation(async (saltRounds: number = 10) => {
  ...
  return {
    ...
    mockHashSuccess: (password: string) => {
