...
export const createMockNodemailer = () => {
  ...
  const resetEmails = () => { sentEmails.length = 0; };
  const getLastEmail = () => sentEmails[sentEmails.length - 1] || null;
  const getEmailsTo = (recipient: string) => sentEmails.filter(email => email.to === recipient);
...
export const { createTransport, getTestTransport, getEmails, resetEmails } = createMockNodemailer();